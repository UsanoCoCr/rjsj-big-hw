...
ValuePtr divi(const std::vector<ValuePtr>& params, EvalEnv& Env){
    ...
    if(params.size() == 1 && dynamic_cast<NumericValue&>(*params[0]).asNumber() == 0){
        throw LispError("div function cannot divide by zero.");
    }
    if(params.size() == 2 && dynamic_cast<NumericValue&>(*params[1]).asNumber() == 0){
        throw LispError("div function cannot divide by zero.");
    }
    double result = 1.000000;
    if(params.size() == 1){
    ...
    result = dynamic_cast<NumericValue&>(*params[0]).asNumber()*1.0/dynamic_cast<NumericValue&>(*params[1]).asNumber();
    return std::make_shared<NumericValue>(result);
}
...
ValuePtr reduce(const std::vector<ValuePtr>& params, EvalEnv& Env){
    ...
    return std::make_shared<PairValue>(Env.apply(proc, list->carValue()->toVector()),
...
ValuePtr greatereq(const std::vector<ValuePtr>& params, EvalEnv& Env){
    ...
    if(dynamic_cast<NumericValue&>(*params[0]).asNumber() >= dynamic_cast<NumericValue&>(*params[1]).asNumber()){
        return std::make_shared<BooleanValue>(true);
    }
    return std::make_shared<BooleanValue>(false);
}
